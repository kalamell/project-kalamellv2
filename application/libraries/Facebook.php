        ...
        if ($session)
        {
            try
            {
                ...
                $fb_user = new FacebookRequest($session, 'GET', '/me');
                    $user = $fb_user->execute()->getGraphObject()->asArray();
    ...
    public function user()
    {
        ...
        if ($session)
        {
            try
            {
                ...
                $fb_user2 = new FacebookRequest($session, 'GET', '/me/permissions');
                $user['permissions'] = $fb_user2->execute()->getGraphObject()->asArray();
    ...
    **/
    public function get_post($id = NULL)
    {
        ...
        if ($session)
        {
            ...
            {
                // Get post data
                $fb = new FacebookRequest($session, 'GET', '/'.$id);
                    $post = $fb->execute()
                    ->getGraphObject()
                    ->asArray();
    ...
    **/
    public function publish_text($message = '')
    {
        ...
        if ($session)
        {
            ...
            {
                // Publish post
                $fb = new FacebookRequest($session, 'POST', '/me/feed',
                        array(
                            'message' => $message
                        )
                    );
                $response = $fb->execute()->getGraphObject()->asArray();
    ...
    **/
    public function publish_video($file = '', $description = '', $title = '')
    {
        ...
        if ($session)
        {
            ...
            {
                // Publish video
                $fb = new FacebookRequest($session, 'POST', '/me/videos',
                        array(
